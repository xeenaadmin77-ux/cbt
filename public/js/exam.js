...
(function() {
  ...
  async function initExamSession(examId) {
    ...
    try {
      ...
      // Calculate clock offset between client device and server
  ...
  // 2. Server-Authoritative Countdown Timer
  ...
  function renderQuestion(index) {
    ...
    // Manage Per-Question Timer (প্রতি প্রশ্নের জন্য timer সেট)
    ...
    if (timeLimit > 0 && qTimerBox && qTimerCount) {
      ...
      questionTimerInterval = setInterval(() => {
        ...
        if (questionTimeRemaining <= 0) {
          clearInterval(questionTimerInterval);
          questionTimerInterval = null;
          // Time expired for this question: Auto-advance to next question or submit if on last question
  ...
  // 4. Select Answer & Auto-save
  async function selectAnswer(questionId, optionKey) {
    if (isSubmitting) return;

    // Toggle or select
    userAnswers[questionId] = optionKey;
    renderQuestion(currentIndex); // Re-render to highlight selected card
    ...
    try {
      ...
      showAutoSaveStatus('⚠️ Save pending (connection retry)');
  ...
  // 8. Custom Close / Early Exit Exam Dialog with Teacher Authorization Password
