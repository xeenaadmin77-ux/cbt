...
function renderResultsList(results) {
  ...
  if (results.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center; padding: 24px; color: var(--text-muted);">কোনো ফলাফল পাওয়া যায়নি (No student records match criteria).</td></tr>';
    return;
  }
  ...
  modal.innerHTML = `
    <div class="modal-dialog" style="max-width: 680px;">
      <div class="modal-header">
        <h3>Student Examination Scorecard (বিস্তারিত মূল্যায়ন)</h3>
      </div>
...
function parseBulkStudentsData(rawText) {
  ...
  for (let i = startIndex; i < lines.length; i++) {
    const line = lines[i];
    // Split by comma or tab (for Excel paste)
...
function renderStudentRoster(students) {
  ...
  if (students.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 24px; color: var(--text-muted);">কোনো শিক্ষার্থী তালিকাভুক্ত নেই (No students match search).</td></tr>';
    return;
  }
...
function exportRosterToCsv() {
  ...
  const headers = ["Candidate Full Name", "Portal Username", "Admission Form Number (Password)", "Trade", "Academic Batch"];
...
// ----------------------------------------------------
// 6. Settings & Invigilator Configuration
// ----------------------------------------------------
async function initSettingsManager() {
  ...
  try {
    const res = await apiFetch('/teacher/settings');
    ...
    document.getElementById('cfgExitPassword').value = s.exitPassword || '';
  } catch (err) {
    console.error(err);
  }
...
async function loadAuditLogs() {
  ...
  try {
    ...
    tbody.innerHTML = logs.map(l => `
      <tr>
        <td><span class="badge ${l.eventType.includes('warn') || l.eventType.includes('exit') ? 'badge-danger' : 'badge-primary'}">${escapeHtml(l.eventType)}</span></td>
