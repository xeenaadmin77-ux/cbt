...
const ExamSecurity = (function() {
  ...
  function bindWindowBlurDetection() {
    window.addEventListener('blur', () => {
      if (isSubmittingOrExiting) return;
      // Slight debounce so select dropdowns or native dialogs don't false trigger
      setTimeout(() => {
  ...
  function bindBeforeUnloadProtection() {
    window.addEventListener('beforeunload', (e) => {
      if (isSubmittingOrExiting) return;
      // Triggers standard browser confirmation: "Changes that you made may not be saved"
      e.preventDefault();
  ...
  async function requestFullscreenMode() {
    ...
    try {
      ...
      console.warn('Fullscreen request bypassed by user or permissions:', err);
    }
  }
